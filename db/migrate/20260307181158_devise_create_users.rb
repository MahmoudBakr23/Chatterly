...
class DeviseCreateUsers < ActiveRecord::Migration[8.1]
  def change
    create_table :users do |t|
      ...
      # null: false — every user must have one. Lowercase only (model enforces this).
    ...
    # Index on last_seen_at: serves "all users where last_seen_at > 5.minutes.ago" presence queries.
    # Without it, that scans the entire users table on every presence check.
